...
def extract_run_accession(runs_text: str) -> str:
    """Extrae el accession del run del texto XML/string"""
    if not runs_text or runs_text == "N/A":
        return "N/A"
    
    # Si es XML, extraer el valor del atributo acc
    if runs_text.startswith('<'):
        match = re.search(r'acc="([^"]+)"', runs_text)
        return match.group(1) if match else "N/A"
    else:
        # Si es texto simple, tomar el primer elemento
        runs = runs_text.split(",")
        return runs[0].strip() if runs else "N/A"
# =================================================== #
    ...
    try:
        ...
        for sra_id in ids:
            try:
                ...
                # Extraer el run accession correctamente
                runs_text = summary.get("Runs", "N/A")
                run_accession = extract_run_accession(runs_text)
        ...
        df = df[df["RunAccession"] != "N/A"]
        
        df.to_csv(output, index=False)
        print(f"Saved SRA sample info to {output} ({len(df)} valid runs)")
...
def download_and_concat_sra_grouped(sra_df: pd.DataFrame, sra_dir: str, concat_dir: str):
    ...
    for _, row in sra_df.iterrows():
        sample_name = row['Title']
        run_accession = row['RunAccession']
        grouped[sample_name].append(run_accession)
        layout_map[sample_name] = row['LibraryLayout']
    ...
    for sample_name, runs in grouped.items():
        ...
        for run in runs:
            ...
            if not file_exists(r1_path) or (r2_path and not file_exists(r2_path)):
                ...
                try:
                    subprocess.run(["prefetch", "--output-directory", sra_dir, run], check=True, capture_output=True)
                    print(f"Converting {run} to FASTQ.gz ...")
                    subprocess.run(["fastq-dump", run, "-O", run_dir, "--split-files", "--gzip"], check=True, capture_output=True)
        ...
        def concat_gz(files, output_file):
            ...
            if file_exists(output_file):
                ...
                return
            print(f"Concatenating {len(files)} files to {output_file}")
            try:
        ...
        if r1_files:
            out_r1 = os.path.join(concat_dir, f"{safe_sample_name}_1.fastq.gz")
            concat_gz(r1_files, out_r1)
        if layout_map[sample_name] == "PAIRED" and r2_files:
            out_r2 = os.path.join(concat_dir, f"{safe_sample_name}_2.fastq.gz")
...
def main():
    ...
    if args.download_all or args.concat:
        if sra_df is not None and not sra_df.empty:
